...
# Define bins for order size ranges based on items_count
bins = [0, 1, 2, 5, 10, 15, float('inf')]
labels = ['0', '1', '2-5', '6-10', '11-15', '15>']

# Create a new column for order size range
vekto_quote_has_customer_email['order_size_range'] = pd.cut(vekto_quote_has_customer_email['items_count'], bins=bins, labels=labels, right=False)
